...
| Fecha        | Cambios                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Junio 2026   | **Fix login mozos — timeout:** `autenticarMozo` en `mozos.repository.js` dejó de hacer `find({})` en cada login; búsqueda acotada por `name`/`usuarioWeb`/`nombres` con fallback al escaneo completo. Evita timeouts del cliente con muchos mozos en BD. Ver [ENTREGA_PLEXPERITY_JUNIO_2026.md](../ENTREGA_PLEXPERITY_JUNIO_2026.md). |
| Mayo 2026    | **Push notifications operativas:** `pushNotifications.js` integrado con `events.js` y controladores de comanda/procesamiento. Envío vía Expo Push API cuando plato o comanda pasa a `recoger`. Deduplicación 10 s. Emisión Socket dirigida a room `mozo-{mozoId}`. |
| Abril 2026   | **Higiene de despliegue:** `.env` y `data/*.json` dejaron de versionarse; `.env.example` añadido. Ajustes de arranque en `index.js` para modo operativo. Plan launcher Windows en `docs/PLAN_LAUNCHER_NATIVO_WINDOWS.md`. |
